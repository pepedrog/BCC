...
pos[0]pos[1]: em sentido horário e a partir da (nossa) direita essas seriam:
pos[0][(pos[1+1)%N], [((pos[0]+1)%N]pos[1], pos[0][(pos[1]-1)%N] e [((pos[0]-1)%N]pos[1]. Cada entrada
mundopos[0]pos[1] é uma lista de anotações/rótulos correspondentes às
...
def planejar(percepcao):
    ...
    if __DEBUG__:
        ...
        pos = posicao
        ori = orientacao
        if ([] == percepcao) or (["Dummy"] == percepcao):
            mundo[pos[0]][pos[1]] = ["V"]
            if mundo[(pos[0]+1)%len(mundo)][pos[1]] == []:
                mundo[(pos[0]+1)%len(mundo)][pos[1]] = ["L"]           
            if mundo[(pos[0]-1)%len(mundo)][pos[1]] == []:
                mundo[(pos[0]-1)%len(mundo)][pos[1]] = ["L"]
            if mundo[pos[0]][(pos[1]+1)%len(mundo)] == []:
        ...
        if "F" in percepcao:
            if "B" not in percepcao:
                if mundo[(pos[0]+1)%len(mundo)][pos[1]] == ["P?"]:
                    mundo[(pos[0]+1)%len(mundo)][pos[1]] = ["L"]
                if mundo[(pos[0]-1)%len(mundo)][pos[1]] == ["P?"]:
                    mundo[(pos[0]-1)%len(mundo)][pos[1]] = ["L"]
                if mundo[pos[0]][(pos[1]+1)%len(mundo)] == ["P?"]:
                    mundo[pos[0]][(pos[1]+1)%len(mundo)] = ["L"]
                if mundo[pos[0]][(pos[1]-1)%len(mundo)] == ["P?"]:
                    mundo[pos[0]][(pos[1]-1)%len(mundo)] = ["L"]
            if mundo[pos[0]][pos[1]] != ["V"]:
                if mundo[(pos[0]+1)%len(mundo)][pos[1]] == []:
                ...
                if mundo[pos[0]][(pos[1]-1)%len(mundo)] == []:
                    mundo[pos[0]][(pos[1]-1)%len(mundo)] = ["W?"]
                mundo[pos[0]][pos[1]] = ["V"]
            #bloco que testa se encontra qual W? realmente é W
            if (mundo[(pos[0]+1)%len(mundo)][pos[1]] == ["W?"]) and (mundo[(pos[0]-1)%len(mundo)][pos[1]] != ["W?"]) and (mundo[pos[0]][(pos[1]+1)%len(mundo)] != ["W?"]) and (mundo[pos[0]][(pos[1]-1)%len(mundo)] != ["W?"]) :
                mundo[(pos[0]+1)%len(mundo)][pos[1]] = ["W"]
            if (mundo[(pos[0]+1)%len(mundo)][pos[1]] != ["W?"]) and (mundo[(pos[0]-1)%len(mundo)][pos[1]] == ["W?"]) and (mundo[pos[0]][(pos[1]+1)%len(mundo)] != ["W?"]) and (mundo[pos[0]][(pos[1]-1)%len(mundo)] != ["W?"]) :
                mundo[(pos[0]-1)%len(mundo)][pos[1]] = ["W"]
            if (mundo[(pos[0]+1)%len(mundo)][pos[1]] != ["W?"]) and (mundo[(pos[0]-1)%len(mundo)][pos[1]] != ["W?"]) and (mundo[pos[0]][(pos[1]+1)%len(mundo)] == ["W?"]) and (mundo[pos[0]][(pos[1]-1)%len(mundo)] != ["W?"]) :
        ...
        if "B" in percepcao:
            if "F" not in percepcao:
                if mundo[(pos[0]+1)%len(mundo)][pos[1]] == ["W?"]:
                    mundo[(pos[0]+1)%len(mundo)][pos[1]] = ["L"]
                if mundo[(pos[0]-1)%len(mundo)][pos[1]] == ["W?"]:
                    mundo[(pos[0]-1)%len(mundo)][pos[1]] = ["L"]
                if mundo[pos[0]][(pos[1]+1)%len(mundo)] == ["W?"]:
                    mundo[pos[0]][(pos[1]+1)%len(mundo)] = ["L"]
                if mundo[pos[0]][(pos[1]-1)%len(mundo)] == ["W?"]:
                    mundo[pos[0]][(pos[1]-1)%len(mundo)] = ["L"]
            if mundo[pos[0]][pos[1]] != ["V"]:
                ...
                if mundo[pos[0]][(pos[1]-1)%len(mundo)] == []:
                    mundo[pos[0]][(pos[1]-1)%len(mundo)] = ["P?"]
                mundo[pos[0]][pos[1]] = ["V"]
            if (mundo[(pos[0]+1)%len(mundo)][pos[1]] == ["P?"]) and (mundo[(pos[0]-1)%len(mundo)][pos[1]] != ["P?"]) and (mundo[pos[0]][(pos[1]+1)%len(mundo)] != ["P?"]) and (mundo[pos[0]][(pos[1]-1)%len(mundo)] != ["P?"]) :
                mundo[(pos[0]+1)%len(mundo)][pos[1]] = ["P"]
            if (mundo[(pos[0]+1)%len(mundo)][pos[1]] != ["P?"]) and (mundo[(pos[0]-1)%len(mundo)][pos[1]] == ["P?"]) and (mundo[pos[0]][(pos[1]+1)%len(mundo)] != ["P?"]) and (mundo[pos[0]][(pos[1]-1)%len(mundo)] != ["P?"]) :
                mundo[(pos[0]-1)%len(mundo)][pos[1]] = ["P"]
            if (mundo[(pos[0]+1)%len(mundo)][pos[1]] != ["P?"]) and (mundo[(pos[0]-1)%len(mundo)][pos[1]] != ["P?"]) and (mundo[pos[0]][(pos[1]+1)%len(mundo)] == ["P?"]) and (mundo[pos[0]][(pos[1]-1)%len(mundo)] != ["P?"]) :
            ...
            if (mundo[(pos[0]+1)%len(mundo)][pos[1]] != ["P?"]) and (mundo[(pos[0]-1)%len(mundo)][pos[1]] != ["P?"]) and (mundo[pos[0]][(pos[1]+1)%len(mundo)] != ["P?"]) and (mundo[pos[0]][(pos[1]-1)%len(mundo)] == ["P?"]) :
                mundo[pos[0]][(pos[1]-1)%len(mundo)] = ["P"] 
            #bloco que testa se encontra qual P? que realmente é P
...
def agir():
    ...
    pos = posicao
    ori = orientacao
    #matriz para movimentar personagem em direção a sala livre
    movimento = []
    for x in range(len(mundo)):
        linha = []
        for y in range(len(mundo[0])):
            if mundo[x][y]=="W?" or mundo[x][y]=="P?" or mundo[x][y]=="P" or mundo[x][y]=="M":
                linha.append("-1")
            if mundo[x][y]=="W":
                linha.append("K")
            if mundo[x][y]=="L":
                linha.append("1")
            movimento.append(linha)
    #matriz para marcar os passos
    for m in range(len(movimento)):
        for n in range(len(movimento[0])):
            if movimento[m][n] == "1":
                movimento[pos[0]][pos[1]] =  min(((pos[0]+pos[1])-(m+n))%len(movimento), ((m+n)-(pos[0]+pos[1]))%len(movimento))
                
            
            
        
            
    if mundo[pos[0]+ori[0]][pos[1]+ori[1]] == "L":
        acao = "A"
